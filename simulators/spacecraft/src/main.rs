...
#[tokio::main]
async fn main() {
    let app = Router::new().route("/health", get(health)).route("/simulations/start", post(start_simulation));

    let address = SocketAddr::from(([127, 0, 0, 1], 8090));

    println!("Spacecraft simulator listening on http://{address}");

    let listener = tokio::net::TcpListener::bind(address).await.expect("failed to bind simulator address");

    axum::serve(listener, app).await.expect("simulator server failed");
}
...
async fn start_simulation(Json(request): Json<StartSimulationRequest>,) -> Json<StartSimulationResponse> {
